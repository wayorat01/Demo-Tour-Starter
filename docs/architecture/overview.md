...
- Use lowercase with dashes for directories. Favor named exports for components

### TypeScript & React 19 Standards
- Use TypeScript for all code; prefer interfaces over types
- Avoid unnecessary curly braces in conditionals; use concise syntax for simple statements
- **React 19:** DO NOT use `forwardRef`. In React 19, `ref` is passed as a regular prop

### Payload CMS Essential Patterns & Security (CRITICAL)
- **Local API Access Control:** You MUST explicitly pass `overrideAccess: false` when querying on behalf of a user to enforce permissions
- **Transactions in Hooks:** ALWAYS pass the `req` object to nested database operations inside hooks (e.g., `req.payload.create({ ..., req })`) to maintain transaction atomicity
- **Prevent Infinite Hook Loops:** Use `context: { skipHooks: true }` to prevent hooks from triggering themselves continuously
- **Data Fetching:** Always use `getPayload({ config })` in Next.js Server Components and API Routes

### UI, Styling & Forms (Tailwind v4)
- Use Shadcn UI and Tailwind v4 for styling. Mobile-first approach
- **Tailwind v4 Rules:**
  - DO NOT use `tailwind.config.ts`. Configure via CSS-first `@theme` blocks
  - Use `@import "tailwindcss"` instead of `@tailwind`
  - Use the new `size-*` utilities instead of `w-* h-*`
  - For dark mode, use `@custom-variant dark (&:where(.dark, .dark *))`
- Always use Lexical Editor for Payload CMS rich text fields
- For UI quality assurance, use the `ui-ux-pro-max` skill (`.agents/skills/ui-ux-pro-max/`)
