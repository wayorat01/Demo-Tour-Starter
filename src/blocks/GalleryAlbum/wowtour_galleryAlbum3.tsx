...
import './wowtour_galleryAlbum3.css'

type WowtourGalleryAlbum3Props = WowtourGalleryAlbumBlock & {
  publicContext: PublicContextProps
}

export const WowtourGalleryAlbum3: React.FC<WowtourGalleryAlbum3Props> = ({
  headingSettings,
  albums,
  cardSettings,
  buttonSettings,
  ...
  const renderAlbumCard = (album: GalleryAlbum, index: number) => {
    ...
    const cardContent = (
      <div className="gallery-cards__card-wrap" style={{ borderRadius: `${borderRadius + 6}px` }}>
        <div className="gallery-cards__card" style={{ borderRadius: `${borderRadius}px` }}>
          <div className="gallery-cards__image">
            {coverImage && (
              <Media resource={coverImage} fill imgClassName="object-cover w-full h-full" />
            )}

            {/* Title bar at bottom of image */}
            <div className="gallery-cards__title-bar">
              <span className="gallery-cards__title">{album.title}</span>
            </div>

            {/* Hover overlay — magnifying glass */}
            <div className="gallery-cards__overlay">
              <Search className="gallery-cards__search-icon" strokeWidth={2} />
            </div>
          </div>
        </div>
      </div>
  ...
  const renderSlideMode = () => (
    <div className="relative overflow-visible">
      ...
      >
        <CarouselContent className="-ml-2 py-2 md:-ml-3">
  ...
  return (
    <section className="w-full py-12 md:py-16">
      <div className="container">
        ...
        {showButton && (
          <div className="gallery-cards__button-wrap">
            <Link href={buttonLink} className="gallery-cards__button" prefetch={false}>
              {buttonText}
            </Link>
          </div>
        )}
...
export default WowtourGalleryAlbum3
