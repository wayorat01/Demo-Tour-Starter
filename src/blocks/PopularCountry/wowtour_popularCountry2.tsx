// Design Version 2: Grid layout with 4:3 ratio rectangular images
// Image Aspect Ratio: 4:3
// Default Border Radius: 8px (small rounded corners)
// Max Items Before Overflow: 8
...
type WowtourPopularCountry2Props = WowtourPopularCountryBlock & {
  publicContext: PublicContextProps
}

const MAX_VISIBLE_ITEMS = 8

export const WowtourPopularCountry2: React.FC<WowtourPopularCountry2Props> = ({
  headingSettings,
  countries,
  imageSettings,
  ...
  const borderRadius = imageSettings?.borderRadius ?? 8
  const displayMode = 'showAll' // V2 always uses showAll mode
  ...
  const shouldUseShowAll = hasOverflow && (displayMode as string) === 'showAll'
  ...
  const renderCountryItem = (country: (typeof countries)[0], index: number) => {
    ...
    const content = (
      <div className="flex flex-col items-center gap-3">
        ...
        <div
          className="group relative w-full cursor-pointer overflow-hidden transition-all duration-300"
          style={{
        ...
        {/* Country name */}
        {tourTitle && (
          <span className="text-center text-sm font-medium md:text-base">{tourTitle}</span>
        )}
      </div>
    )
      ...
      >
        <CarouselContent className="-ml-3 md:-ml-4">
          {countries.map((country, index) => (
            <CarouselItem key={index} className="basis-1/2 pl-3 sm:basis-1/3 md:basis-1/4 md:pl-4">
  ...
  const renderShowAllMode = () => (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="grid w-full grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 md:gap-6">
      ...
      <div className="container">
        ...
        <div className="mb-8 flex flex-col items-center text-center md:mb-12">
          <div className="mb-2 flex items-center gap-3">
            ...
            <h2 className="text-2xl font-medium">{heading || 'ประเทศยอดนิยม'}</h2>
          </div>
...
export default WowtourPopularCountry2
