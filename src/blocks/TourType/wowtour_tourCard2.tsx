...
import './wowtour_tourCard2.css'

export type TourItem = any & {
  interTourSlug?: string | null
}

type WowtourTourCard2Props = Omit<WowtourTourTypeBlock, 'tourProgramRefs'> & {
  tours: TourItem[]
      ...
      <div className="tour2-card" style={{ borderRadius: `${borderRadius}px` }}>
        ...
        >
          ...
          {toggle?.showTourCode && tour.tourCode && (
            <span className="tour2-tour-code">รหัสทัวร์ {tour.tourCode}</span>
          )}

          {/* Recommended badge — top right */}
          {toggle?.showRecommendedTag && tour.recommendedLabel && (
            <span className="tour2-recommended">{tour.recommendedLabel}</span>
          )}
...
export const WowtourTourCard2: React.FC<WowtourTourCard2Props> = ({
  headingSettings,
  apiTitle,
  tours,
  cardSettings,
  ...
  const renderGridMode = () => (
    <>
      <div
        className={visibleTours.length < effectiveCols ? 'flex flex-wrap justify-center' : 'tour2-grid'}
...
export default WowtourTourCard2
