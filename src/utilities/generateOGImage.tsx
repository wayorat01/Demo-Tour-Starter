 ...
 */
function isLocalizedMedia(value: unknown): value is { th?: Media | string | null } {
...
export default async function generateOGImage({
  ...
  const bgImage = pageConfig.siteIdentity?.sitePreviewImage
  ...
  // Extract the localized page title (uses siteIdentity.siteName as fallback)
  const siteIdentity = pageConfig.siteIdentity as any
  const pageTitle = title || siteIdentity?.siteName || 'Payblocks'

  const textColor =
    siteIdentity?.ogTextColor || (pageConfig as any).openGraph?.textColor || '#000000'
