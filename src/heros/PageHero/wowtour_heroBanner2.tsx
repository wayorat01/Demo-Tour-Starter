...
type WowtourHeroBanner2Props = Page['hero'] & { publicContext: PublicContextProps; pageTitle?: string; preloadedGlobalSettings?: any; preloadedSearchOptions?: any }


/**
 * WowtourHeroBanner2 (Hero version)
 * Desktop: Search Box (left, 350px) + Banner Slide (right, flex-1) — 600px tall
 * Mobile:  Banner Slide (top, 220px) then Search Box (bottom, auto height)
 */
export const WowtourHeroBanner2: React.FC<WowtourHeroBanner2Props> = ({
  sliderImages,
  autoPlayDelay = '20000',
  searchBgType,
  searchBgColor,
  ...
  // Build section background style from search box settings
  const getSectionBgStyle = (): React.CSSProperties => {
    const bgType = searchBgType || 'color'
    if (bgType === 'gradient') {
      const startColor = searchGradientStartColor || 'hsl(173, 100%, 46%)'
      const endColor = searchGradientEndColor || 'hsl(214, 97%, 61%)'
      const gradType = searchGradientType || 'linear'
      const gradPos = searchGradientPosition || 'to right'
      if (gradType === 'radial') {
        const radialPos = gradPos.replace('to ', 'at ')
        return { background: `radial-gradient(circle ${radialPos}, ${startColor}, ${endColor})` }
      }
      return { background: `linear-gradient(${gradPos}, ${startColor}, ${endColor})` }
    }
    if (bgType === 'color') {
      return { backgroundColor: searchBgColor || 'transparent' }
    }
    return {}
  }
  ...
  return (
    <section className="relative w-full" style={getSectionBgStyle()}>
      {/* Background image if type is image */}
      {searchBgType === 'image' && searchBgImage && (
        <div className="absolute inset-0">
          <Media resource={searchBgImage as MediaType} fill imgClassName="object-cover" />
        </div>
      )}

      <div className="relative z-10 container py-[50px]">
        {/*
                    Responsive layout:
                    - Mobile: flex-col (banner top → search bottom)
                    - Desktop (lg+): flex-row (search left → banner right)
                */}
        <div className="flex flex-col gap-3 lg:flex-row lg:gap-4">
          ...
          {/* Banner Slide Carousel — order-1 on mobile (top), order-2 on desktop (right) */}
