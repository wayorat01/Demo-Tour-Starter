...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (mode === 'import') {
      ...
      // ไม่มี Fallback category — ถ้าไม่ได้ map category ให้ปล่อยว่าง
    }

    // ============================================
    // Step 2: Create / Update Inbound Tour posts
    // ============================================
      ...
      if (mode === 'import') {
        ...
        if (payloadData.parentCountry) {
          ...
          if (parentDoc.docs.length > 0) {
            ...
            // ใช้ category ของประเทศหลักเสมอ
            ...
            try {
              ...
              console.warn(`[import-inbound] Could not create parent "${parentNameTH}":`, e.message)
        ...
        // ลบ parentCountry_en ออกจาก payloadData
    ...
    console.error('Import inbound error:', error)
