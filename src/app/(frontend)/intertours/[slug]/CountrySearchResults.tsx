...
function SearchDropdown({
    ...
    searchAliases?: Record<string, string>   // option -> alternate search text (e.g. English name)
}) {
    ...
    const filtered = options.filter((o) => {
        ...
        // Also search by alias (e.g. English name, slug)
...
function SearchFieldsLayoutCountry({
    ...
    // Country selector — uses SearchDropdown matching home/search-tour pattern
            ...
            <div key="location" className="w-full min-w-0">
                <SearchDropdown
                    ...
                    onChange={(val) => {
                        ...
                        if (match?.slug) {
                            router.push(`/intertours/${match.slug}`)
                        }
                    }}
    ...
    return (
        <div className="flex flex-col gap-4 md:gap-5">
            ...
            {hasExtra && (
                ...
                >
                    <div className="relative border border-input/50 rounded-xl py-4 md:py-5 px-0 bg-transparent">
                        <button
...
export default function CountrySearchResults({
    ...
    const [festivalSlug, setFestivalSlug] = useState(searchParams.get('festivals') || '') // Added this line
    ...
    const countryOptions = ssrOptionsData?.countryList || []
    ...
    const [sidebarFilterConfig, setSidebarFilterConfig] = useState<SidebarFilterConfig>({ enabled: true })
    ...
    const basePath = `/intertours/${countrySlug}`

    // Use ssrOptionsData directly for filter options
    useEffect(() => {
        if (ssrOptionsData?.filterOptions) {
            setFilterOptions(ssrOptionsData.filterOptions)
        }
    }, [ssrOptionsData])
    ...
    useEffect(() => {
        ...
        const festivalParam = searchParams.get('festival') // Legacy single festival param
        const airlinesParam = searchParams.get('airlines')
        const cityParam = searchParams.get('city')
        const monthParam = searchParams.get('month')

        const newPriceMin = priceMinParam ? Number(priceMinParam) : (dbPriceRange?.min ?? 0)
        const newPriceMax = priceMaxParam ? Number(priceMaxParam) : (dbPriceRange?.max ?? 600000)

        const initAirlines = airlinesParam ? airlinesParam.split(',').map(f => f.trim()).filter(Boolean) : []
    ...
    }, [searchParams, dbPriceRange, ssrOptionsData])
