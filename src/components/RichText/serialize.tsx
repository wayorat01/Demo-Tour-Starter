...
export type NodeTypes =
  ...
  | SerializedBlockNode<
      ...
      | CodeBlockProps
    >
  | { type: 'table'; children: NodeTypes[]; [key: string]: unknown }
  | { type: 'tablerow'; children: NodeTypes[]; [key: string]: unknown }
  | {
      type: 'tablecell'
      children: NodeTypes[]
      headerState?: number
        ...
        if (node.type === 'block') {
          ...
          switch (typeString) {
            ...
            case 'heading': {
              ...
              return (
                ...
                <Tag
                  className={className}
                  key={index}
                  id={getHeadlineId(nodeAny, index.toString())}
                >
                  {serializedChildren}
                </Tag>
              )
              ...
              return (
                <Tag
                  className={cn(
                    ...
                    nodeAny.listType === 'bullet' ? 'list-disc' : 'list-decimal',
                  )}
                  key={index}
                >
                  {serializedChildren}
            ...
            case 'listitem': {
              ...
              if (nodeAny?.checked != null) {
                return (
                  <li
                    aria-checked={nodeAny.checked ? 'true' : 'false'}
                    className={className}
                    key={index}
                    role="checkbox"
                    tabIndex={-1}
                ...
                return (
                  <li key={index} className={className} value={nodeAny?.value}>
                    {serializedChildren}
                  </li>
                )
              }
            ...
            case 'upload': {
              const media = nodeAny.value as MediaType
              ...
              // Read custom fields from upload node
              const uploadFields = (node as any).fields || {}
              const imageSize = uploadFields.imageSize || '100'
              const imageHAlign = uploadFields.imageHAlign || 'left'
              const imageVAlign = uploadFields.imageVAlign || 'top'

              // When imageSize is 100%, show at original size (no forced width)
              // When imageSize < 100%, constrain with maxWidth
              const imageSizeStyle: React.CSSProperties =
                imageSize !== '100' ? { maxWidth: `${imageSize}%` } : {}
              ...
              return (
                <div className={cn('col-start-2 my-4 flex', hAlignClass, vAlignClass)} key={index}>
                  <figure className="relative" style={imageSizeStyle}>
                    <Image
                      src={url}
                      alt={alt || ''}
                      width={width || 800}
            ...
            case 'table': {
              return (
                <table key={index} className="col-start-2">
            ...
            case 'tablecell': {
              ...
              const rowSpan = cellNode.rowSpan || 1
              // Parse inline styles from the cell node (e.g. vertical-align set via toolbar)
              const cellStyle: React.CSSProperties = {}
              if (cellNode.style) {
                const styleStr = cellNode.style as string
                styleStr.split(';').forEach((rule: string) => {
                  ...
                  if (prop && val) {
                    ...
                    ;(cellStyle as any)[camelProp] = val
                  }
                })
              }
              return (
