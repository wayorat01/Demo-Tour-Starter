...
const Navbar5: React.FC<{ header: HeaderType; publicContext: PublicContextProps; citiesMap?: Record<string, any[]> }> = ({
  header,
  publicContext,
  citiesMap,
}) => {
                    ...
                    <NavigationMenuItem key={item.id} className="text-muted-foreground">
                      ...
                      <NavigationMenuContent>
                        <ul className="w-80 p-3">
                          {(item as { subitems?: any[] }).subitems?.map((subitem) => (
                    ...
                    {header.items?.map((item: any) => {
                      if (item.blockType === 'link') {
                        ...
                        return (
                          <AccordionItem
                            key={item.id}
                            value={item.id || (item as { label?: string }).label || ''}
