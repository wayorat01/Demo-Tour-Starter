...
import './wowtour_navbar7.css'

// Social platform icons — round style for top bar
const socialRoundIcons: Record<string, React.ReactNode> = {
  facebook: (
    <svg viewBox="0 0 320 512" fill="currentColor" className="h-[12px] w-[12px]">
  ...
  twitter: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="h-[12px] w-[12px]">
      <path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z" />
    </svg>
  ),
  email: <Mail className="h-[14px] w-[14px]" />,
}

export const Navbar7WowTour: React.FC<{
  header: HeaderType
  ...
  const showTourismLicense = topBar?.showTourismLicense !== false

  // Separate email social link from other socials
  const emailSocial = socialLinks.find((s) => s.platform === 'email')
  const nonEmailSocials = socialLinks.filter((s) => s.platform !== 'email')

  return (
    <>
      <div className="wowtour-navbar7-top-wrapper bg-background hidden xl:block">
        {/* ===== Row 1: Top Bar (License + Socials) — Desktop only ===== */}
        <div className="wowtour-navbar7-topbar">
          <div className="container">
            <div className="wowtour-navbar7-topbar-inner">
              {/* Left: TAT Tourism License */}
              <div className="wowtour-navbar7-license">
                {showTourismLicense && companyInfo?.tatLicense && (
                  <span>ใบอนุญาตการท่องเที่ยวแห่งประเทศไทย เลขที่ {companyInfo?.tatLicense}</span>
                )}
              </div>

              {/* Right: Social Icons + Email */}
              <div className="wowtour-navbar7-top-right">
                {/* Social round buttons */}
                {showSocialIcons && nonEmailSocials.map((social: any, idx: number) => {
                  ...
                  return (
                    <a
                      ...
                      className="wowtour-navbar7-social-btn"
                ...
                {/* Email link with icon + text */}
                {showSocialIcons && (emailSocial || companyInfo?.email) && (
                  <a
                    href={`mailto:${emailSocial?.url || companyInfo?.email}`}
                    className="wowtour-navbar7-email-link"
                  >
                    <Mail className="h-4 w-4" />
                    {showSocialLabels && <span>{emailSocial?.url || companyInfo?.email}</span>}
                  </a>
                )}
              </div>
            </div>
        ...
        {/* ===== Row 2: Middle Bar (Hours + Logo + Phones) — Desktop only ===== */}
        <div className="wowtour-navbar7-middle">
          <div className="container">
            <div className="wowtour-navbar7-middle-inner">
              {/* Left: Business Hours */}
              {showBusinessHours && (
                <div className="wowtour-navbar7-hours">
                  <div className="wowtour-navbar7-hours-icon">
                    <Clock className="h-5 w-5" />
                  </div>
                  <div className="wowtour-navbar7-hours-text">
                    <span className="label">เวลาทำการ</span>
                    <span>{companyInfo?.businessHours || 'จันทร์-เสาร์ 09.00 น.-18.00 น.'}</span>
                  </div>
                </div>
              )}

              {/* Center: Logo */}
              <CMSLink publicContext={publicContext} url={'/'} className="wowtour-navbar7-logo">
                {companyInfo?.companyLogo ? (
                  <Media resource={companyInfo.companyLogo} imgClassName="h-14 w-auto" priority />
                ) : header.logo ? (
                  <Media resource={header.logo || undefined} imgClassName="h-14 w-auto" priority />
                ) : null}
              </CMSLink>

              {/* Right: Phone Numbers */}
              {showContacts && (
                <div className="wowtour-navbar7-phones">
                  <div className="wowtour-navbar7-phones-icon">
                    <Phone className="h-5 w-5" />
                  </div>
                  <div className="wowtour-navbar7-phones-text">
                    <span className="label">เบอร์โทรติดต่อ</span>
                    {/* Show multiple phone numbers from phones array, fallback to callCenter */}
                    {(companyInfo?.phones?.length || 0) > 0 ? (
                      companyInfo?.phones?.map((phone: any, idx: number) => (
                        <a
                          key={phone.id || idx}
      ...
      <StickyNavDetector className="wowtour-navbar7-sticky-wrapper z-50">
        {/* ===== Row 3: Nav Bar (Centered Menu — Sticky) ===== */}
        <div className="wowtour-navbar7-sticky">
          <div className="wowtour-navbar7-navstrip">
            <div className="container">
              <div className="wowtour-navbar7-navstrip-inner">
                <NavActiveMarker>
                  <nav className="wowtour-navbar7-nav">
                    <NavigationMenu>
                      <NavigationMenuList>
                            ...
                            if (item.blockType === 'link') {
                              return (
                                <CMSLink
                                  ...
                                  key={item.id}
                                  {...item.link}
                                  className="wowtour-navbar7-nav-link"
                                />
                              )
                            } else if ((item as { blockType?: string }).blockType === 'sub') {
                              return (
                                <NavigationMenuItem key={item.id}>
                                  <NavigationMenuTrigger className="wowtour-navbar7-nav-link">
                              ...
                              if (isCompact) {
                                return (
                                  ...
                                  >
                                    <button className="wowtour-navbar7-nav-link">
                                      {'icon' in item && item.icon && (
